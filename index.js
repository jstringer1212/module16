...
function logItemNames() {
  ...
  // items.forEach((items.name) => {console.log("Name: ", items.name);});
...
function capitalizeNames() {
  ...
  // DO NOT MUTATE THE ORIGINAL ARRAY IN YOU LOGIC  use map?
...
function calculateTotalInventory() {
  ...
  const totalinv = items.reduce((totalinv, item) => totalinv + item.inventory, 0);
  return(totalinv);
}
...
function filterItemsByCategoryId(categoryId) {
  // TODO: use the .filter() method to filter out all items which don't belong in the passed in category
  const category = items.filter((item) => item.categoryId === categoryId);
  return (category.name);
}

function logCartItems() {
  // TODO: Loop through your cart and use the indexes to log the names of all items in your cart
  const cartitems = items.filter((item) => cart.includes(item.id)).map(item => item.name);
